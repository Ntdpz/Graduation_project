<!-- pages/user/User_Management.vue -->
      ...
      <v-col v-for="(user, index) in filteredUsers" :key="index" cols="12" md="4">
        <v-card class="mx-auto" max-width="400" @click="viewDetails(user)">
          <v-img
            ...
            cover
          >
            <v-card-title>{{ user.user_firstname }} {{ user.user_lastname }}</v-card-title>
          </v-img>
  ...
  export default {
    methods: {
      ...
      async refreshUsersData() {
        ...
        this.searchUsers(); // Refresh the filtered users after fetching data
      },
      searchUsers() {
